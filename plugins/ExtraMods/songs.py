...
async def handle_album_download(client, message):
    ...
    if len(songs) > MAX_SONGS_LIMIT:
        await message.reply_text(
            f"The album contains more than {MAX_SONGS_LIMIT} songs. To download more than {MAX_SONGS_LIMIT} songs, please upgrade your account and contact [@TGTesla](https://t.me/TGTesla)."
        )
        return
    ...
    for song in songs:
        song_title = song.get("title").replace(' ', '_') + ".mp3"
...
async def handle_playlist_download(client, message):
    ...
    playlist_title = playlist_data.get("playlistTitle")
    songs = playlist_data.get("songs", [])

    # Check if the playlist exceeds the song limit
    if len(songs) > MAX_SONGS_LIMIT:
        await message.reply_text(
            f"The playlist contains more than {MAX_SONGS_LIMIT} songs. To download more than {MAX_SONGS_LIMIT} songs, please upgrade your account and contact [@TGTesla](https://t.me/TGTesla)."
        )
        return
...
async def handle_song_download(client, message):
    ...
    song_title = song_data.get("title").replace(' ', '_') + ".mp3"
    ...
    await client.send_audio(
        chat_id=message.chat.id,
        audio=file_path,
        caption=f"**{song_data.get('title')}** - {song_data.get('artist')}"
    )
