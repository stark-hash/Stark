...
MEMES = ["https://telegra.ph/file/303b3ca322b5c6ef5187e.jpg"]
# -- Constants End -- #


@Client.on_message(filters.command("cars", COMMAND_HAND_LER) & f_onw_fliter)
async def cars_images(_, message):
    await message.reply_photo(photo = random.choice(CAR_WALLPAPERS))
    caption="Hᴇʀᴇ 😊 !"


@Client.on_message(filters.command("wallpaper", COMMAND_HAND_LER) & f_onw_fliter)
async def wallpaper_scenes(_, message):
    await message.reply_photo(photo = random.choice(SCENERY_LINKS))
    caption="Hᴇʀᴇ 😊 !"


@Client.on_message(filters.command("meme", COMMAND_HAND_LER) & f_onw_fliter)
async def funny_memes(_, message):
    await message.reply_photo(photo = random.choice(MEMES))
    caption="Hᴇʀᴇ 😊 !"
